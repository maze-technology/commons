...
class ProtoValidateGrpcServerInterceptorTest {
  ...
  void interceptCallWithValidMessageShouldProceed() throws ValidationException {
    // Arrange
    ...
    // Act
    final ServerCall.Listener<Message> result = interceptor.interceptCall(
        serverCall,
        headers,
        next
    );
  ...
  void interceptCallWithValidationExceptionShouldRejectWithInternalError()
    ...
    verify(serverCall).close(argThat(status ->
        status.getCode() == io.grpc.Status.INTERNAL.getCode()),
        any(Metadata.class));
  ...
  void interceptCallWithNonProtobufMessageShouldRejectWithInternalError() {
    // Arrange
    ...
    // Act
    final ServerCall.Listener<Object> result = interceptor.interceptCall(
        serverCallForNonProtobuf, headers, nextForNonProtobuf);
  ...
  void onHalfCloseWhenNotRejectedShouldCallDelegate() {
    // Act
    final ServerCall.Listener<Message> result = interceptor.interceptCall(
        serverCall,
        headers,
  ...
  void onCancelWhenNotRejectedShouldCallDelegate() {
    // Act
    final ServerCall.Listener<Message> result = interceptor.interceptCall(
        serverCall,
        headers,
